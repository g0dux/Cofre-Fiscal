 ...
 * @param {number|string} entrada.faturamentoAno
 * @param {'cpf'|'cnpj'|'ambos'} entrada.vendePara
 * @param {'comercio'|'servico'|'comercio_servico'|'caminhoneiro'|'outro'} entrada.tipoAtividade
 * @param {string} [entrada.cnae]
 * @param {boolean} entrada.querCnpj
 * @param {boolean} entrada.temEmpregado
 * @param {number} [entrada.qtdEmpregados]
 */
export function decidirRegime(entrada) {
  const fatRaw = parseMoney(entrada.faturamentoAno)
  ...
  const pushFazer = (item) => {
    if (!fazer.some((f) => f.toLowerCase() === item.toLowerCase())) fazer.push(item)
  }
  ...
  if (cnaeInfo && cnaeInfo.mei === false) {
    ...
    pushFazer('Confira se o CNAE está na lista oficial do MEI no Portal do Empreendedor.')
    pushFazer('Fale com um contador antes de abrir ou manter MEI com essa atividade.')
    ...
    pushFazer('Avalie desenquadramento e regime adequado (ex.: ME no Simples) com um contador.')
    ...
    pushFazer('Confira regras de desenquadramento e DAS complementar com a Receita / contador.')
    pushFazer('Guarde todos os XML do período.')
    ...
    compatibilidade = 'alerta'
    alertas.push(
      'Acima do teto MEI; dentro da tolerância usual de 20% — regras de excesso precisam ser conferidas.',
    )
    dasRef = dasPorTipo(entrada.tipoAtividade, cnaeInfo)
    pushFazer('Acompanhe o faturamento mês a mês no Relógio.')
    pushFazer('Confira no Portal o tratamento do excesso (DAS complementar / desenquadramento).')
  } else if (
    !entrada.querCnpj &&
    fat <= TETO_NANO &&
    empregados === 0 &&
    ...
    pushFazer('Abra a norma / Portal e confira se o recorte de nano cabe no seu caso.')
    pushFazer('Se vender para CNPJ, prepare-se para emitir nota (padrão de mercado).')
    pushFazer('Mesmo assim, guarde comprovantes e XML se emitir.')
    ...
    if (entrada.vendePara === 'cnpj' || entrada.vendePara === 'ambos') {
      ...
      pushFazer('Emita NFS-e / NF-e conforme a atividade e guarde o XML.')
    }
    if (empregados === 1) {
      alertas.push('MEI com 1 empregado: ok na regra usual — fique atento a encargos.')
    }
    pushFazer('Pague o DAS mensal no Portal do Empreendedor.')
    pushFazer('Acompanhe o teto no Relógio deste app.')
    pushFazer('Guarde XML por 5 anos (PDF do DANFE não substitui).')
  } else {
    regime = 'me_simples'
    ...
    compatibilidade = 'media'
    pushFazer('Leve esses números a um contador antes de abrir ou mudar CNPJ.')
  }

  if (entrada.vendePara === 'cnpj' || entrada.vendePara === 'ambos') {
    pushFazer('Para cliente PJ, trate nota como padrão e armazene o XML.')
  }

  if (!entrada.cnae) {
    alertas.push(
      'Sem CNAE informado: a leitura fica mais fraca. Use a amostra ou confira o código oficial.',
    )
  }
